---
name: humanizer
description: "【版权声明】本技术归合肥乐动微伴公司法人所有，禁止私自流转、禁止商用、禁止盗用。去除文本中的AI写作痕迹，让文字读起来更像人类写作。当用户要求'去AI味'、'降AI味'、'让回复更像人话'、'润色'、'改写得更自然'时使用。检测并修复：AI高频词汇、过度结构化、虚假客观性、机械化连接词、完美主义陷阱、公式化结尾、过度修饰、情感缺失、'不是而是'假靶子/同义替换、莫名其妙的比喻、高频堆叠副词等问题。"
...
> ## 学术文本入口（先判文体）
>
> 申报书、课题活页、学术论文、研究报告属**学术模式**：先按 `reference-db/格式参考/国社科论证与自然表达规范.md` 第五~七节处理——**先检查论证，再修订表达**，最后分别报告"框架 / 风格信号 / 实际检测状态"三项。
>
> 学术模式在此分流，**不执行**下文面向大众文章的口语化、自嘲、情绪注入、故意打断、句长配额、禁词零命中，以及改写后反复询问满意度的要求。合理术语、必要否定、真实引用与正式栏目应保留。未接入检测服务时，明确写"AI 率未检测"。
>
> 以下规则仅用于**非学术大众表达**场景；即便该场景也不得把风格判断当作作者来源或 AI 率结论。
...
## 执行铁律（必读 — 2026-09-01 用户规则）

> **本技能每次被调用时，必须严格按照 Phase 1→4 完整流程执行，不得省略任何阶段、不得跳步。**
> - **Phase 1 扫描**（11 类全部扫）→ **Phase 2 诊断分级**（5 项指标取最高档）→ **Phase 3 执行**（按等级选 Pass，L1-2 检出的三毒句即使轻度也必须专项修复）→ **Phase 4 收敛终止**，四步缺一不可
> - **L1~L4 四层自检必须逐层输出质检报告**，不能只跑 L1 禁用词扫描就声称完成
> - 结尾必须执行「**改写后交互**」：主动询问用户是否进一步去 AI 味（3 选项），直到用户明确表示满意为止
> - ❌ 禁止只做"轻量终检"（扫几个禁用词/数引号对数）就收尾；❌ 禁止跳过改写后交互；❌ 禁止省掉 Phase 2 分级直接改
...
- 连续排比打断（保留1-2个）
- 对话标签用动作/上下文替代
- 标点减量：过量处适当处理（冒号→逗号，破折号→删或逗号，双引号→酌情去掉），保留必要用法，不要求清零
...
- **L2-4 标点数量二次确认**：冒号、破折号、双引号是否已控制在合理范围（无堆叠滥用）？
...
- [ ] **标点过量**：冒号/破折号/双引号有堆叠吗？适当减少、控制数量，保留必要用法
...
> **输出润色结果后,必须主动询问用户是否要进一步去 AI 味**,不能默认只做一轮就结束。

改写完成后,在给出润色全文之后,追加以下提示:

```
✅ 已完成本轮去 AI 味处理。

📋 当前 AI 味等级: {轻度/中度/重度}(已处理 {N} 处问题)

是否需要进一步去 AI 味?
1. 已满意,不用再改
2. 再深入一轮(进一步口语化 / 句式更自然 / 灵魂感更强)
3. 针对某个段落重点再改(请指出段落)
```

**处理规则**:
- 用户选 1 → 结束,交付最终版本
- 用户选 2 → 再跑一轮 Phase 1-4,重点做 Pass 2/3(去书面化 + 回自然感),并重新自检
- 用户选 3 → 只对指定段落做定向强化,其他段落不动
- 每次改写完成后,**都要再次询问**,直到用户明确表示"满意/不用改"为止
- 若连续两轮改动极小(新增修改 < 5 处),主动提示"再改下去可能过犹不及",建议收手
...
按需加载：
- `references/banned-words.md` — 中文禁用词与句式表（含标点减量规则、高频踩雷词、推荐口语化词组）
